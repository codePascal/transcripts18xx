...
class PatternHandler(abc.ABC):
    ...
    Class to map transcript lines to a pattern. Each subclass implements ideally
    one pattern, sometimes more, that is compared to a line. The pattern is
    implemented as a regular expression.

    Attributes:
        pattern: The expression that shall be matched to the line.
        type: The type of the pattern, e.g., `bid` or `sell`.
    """
